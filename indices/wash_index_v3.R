...
crit_2_q1_improved_dr <- c('bottled_water','water_kiosk','trucked_in_water','protected_well','protected_spring',
                           ...
                           'tap_drinking_water_piped_into_dwelling', 'technical_piped_water')
...
crit_2_q2_bad <- c('bad')
crit_2_q2_very_bad <- c('very_bad')

crit_2_q3_can_treat <- c('yes')

crit_2_q3_no_no_need_to_treat_the_water <- c('no_no_need_to_treat_the_water')
...
crit_5_q1_level_3 <- c('feminine_hygiene_items','baby_diapers', 'soap', 'water_treatment_product_and_materials')
...
crit_7_q1_enought_water_choices <- c('H_9_enough_water/cooking', 'H_9_enough_water/laundry_and_cleaning',
                                     'H_9_enough_water/personal_hygiene', 'H_9_enough_water/flushing_toilet')
...
crit_7_q2_na <- c('G_16_utility_interrupt/gas','G_16_utility_interrupt/hot_water_supply',
...
crit_7_q2_none <- 'G_16_utility_interrupt/none'


crit_7_q2_relevant <- c('G_16_utility_interrupt/sewage','G_16_utility_interrupt/cold_water_supply')

crit_7_q3_level_2 <- c('H_9_enough_water/laundry_and_cleaning')

crit_7_q3_level_3 <- c('H_9_enough_water/cooking', 'H_9_enough_water/personal_hygiene')
...
crit_9_q1_water_interaptions <- c('G_16_utility_interrupt/sewage', 'G_16_utility_interrupt/hot_water_supply', 'G_16_utility_interrupt/cold_water_supply')
...
data.list$main <- data.list$main %>% 
  ...
  mutate(
    ...
    H_3_1_water_fetch_int  = ifelse(is.na(H_3_1_water_fetch_int ),'0',H_3_1_water_fetch_int ),
    wash_crit_2 = case_when(
      H_2_water_source %in% c('rainwater_collection', 'surface_water') ~ 5,
      
      #### CHANGED PART CRIT 2 START ####
      
      ((H_2_water_source %in% c('unprotected_well', 'unprotected_spring')) &
         !H_6_water_treat  %in% crit_2_q3_can_treat) ~ 4,
      
      (H_2_water_source %in% crit_2_q1_improved_dr &
         H_5_water_assess %in%  c(crit_2_q2_bad, crit_2_q2_very_bad))|
        (H_2_water_source %in% c('unprotected_spring','unprotected_well')&
           H_6_water_treat %in% 'yes') ~ 3,
      
      (H_2_water_source %in% crit_2_q1_improved_dr &
         H_5_water_assess %in% crit_2_q2_acceptable &
         as.numeric(`H_3_1_water_fetch_int`) > 14) |
        (H_2_water_source %in% crit_2_q1_improved_dr &
           H_6_water_treat %in% crit_2_q3_can_treat) ~ 2,
      
      H_2_water_source %in% crit_2_q1_improved_dr &
        (H_5_water_assess %in% crit_2_q2_acceptable) &
        (as.numeric(`H_3_1_water_fetch_int`) < 15) ~ 1,
      ...
      # set default value to 666
      TRUE ~ 666
    ),
    
    #### CHANGED PART CRIT 2 END ####
    ...
    wash_crit_5 = case_when(
      # grepl(paste0('(',paste0(crit_5_q1_level_4, collapse = ')|('),')'),J_25_miss_nfi) ~ 4, deleted
    ...
    `H_9_enough_water/flushing_toilet` = as.numeric(`H_9_enough_water/flushing_toilet`),
    wash_crit_7 = case_when(
      (rowSums(across(all_of(crit_7_q1_improved),.fns = as.numeric), na.rm = T) >0 &
        ...
        (rowSums(across(all_of(crit_7_q1_improved),.fns = as.numeric), na.rm = T) ==0 &
           ...
           ((rowSums(across(all_of(c(crit_7_q3_level_3, crit_7_q3_level_2, "H_9_enough_water/flushing_toilet")),.fns = as.numeric), na.rm = T) > 0 &
               rowSums(across(all_of(c(crit_7_q3_level_3, crit_7_q3_level_2, "H_9_enough_water/flushing_toilet")),.fns = as.numeric), na.rm = T) < 4) |
      ...
      (rowSums(across(all_of(c(crit_7_q1_stressed,crit_7_q1_improved)),.fns = as.numeric), na.rm = T) >0 &
         ...
         ((as.numeric(`H_9_enough_water/flushing_toilet`) == 1 | is.na(`H_9_enough_water/flushing_toilet`)) &
            rowSums(across(all_of(c(crit_7_q3_level_3, crit_7_q3_level_2)),.fns = as.numeric), na.rm = T) < 3)) | 
        ...
        (rowSums(across(all_of(crit_7_q1_improved),.fns = as.numeric), na.rm = T) == 0 &
           ...
           rowSums(across(all_of(c(crit_7_q3_level_3, crit_7_q3_level_2)),.fns = as.numeric), na.rm = T) == 3 &
           (as.numeric(`H_9_enough_water/flushing_toilet`) == 1 | is.na(`H_9_enough_water/flushing_toilet`))) |
        (rowSums(across(all_of(crit_7_q1_improved),.fns = as.numeric), na.rm = T) > 1 &
           as.numeric(crit_7_q3_level_2) == 1 &
           rowSums(across(all_of(c('G_16_utility_interrupt/sewage', 'G_16_utility_interrupt/cold_water_supply')), .fns = as.numeric), na.rm = T) > 0) ~ 3,
      
      #### CHANGED PART START ####
      
      (rowSums(across(all_of(c(crit_7_q1_improved)), .fns = as.numeric), na.rm = T) > 0 &
         (rowSums(across(all_of(c(crit_7_q1_enought_water_choices)),.fns = as.numeric), na.rm = T) == 4|
            (rowSums(across(all_of(c(crit_7_q1_enought_water_choices)),.fns = as.numeric), na.rm = T) == 3&
               is.na(`H_9_enough_water/flushing_toilet`))) &
         rowSums(across(all_of(crit_7_q2_relevant), .fns = as.numeric), na.rm = T) > 0) |
        (rowSums(across(all_of(c(crit_7_q1_improved)), .fns = as.numeric), na.rm = T) > 0 &
           as.numeric(`H_9_enough_water/laundry_and_cleaning`) %in% c(1) &
           rowSums(across(all_of(c('G_16_utility_interrupt/sewage', 'G_16_utility_interrupt/cold_water_supply')), .fns = as.numeric), na.rm = T) == 0 &
           rowSums(across(all_of(crit_7_q2_na), .fns = as.numeric), na.rm = T) > 0) ~ 2,
      ...
      (rowSums(across(all_of(c(crit_7_q1_improved)),.fns = as.numeric), na.rm = T) > 0 &
         (rowSums(across(all_of(c(crit_7_q1_enought_water_choices)),.fns = as.numeric), na.rm = T) == 4|
            (rowSums(across(all_of(c(crit_7_q1_enought_water_choices)),.fns = as.numeric), na.rm = T) == 3&
               is.na(`H_9_enough_water/flushing_toilet`)) ) &
         rowSums(across(all_of(crit_7_q2_none), .fns = as.numeric), na.rm = T) == 1) ~ 1,
    ...
    wash_crit_9 = case_when(
      (as.numeric(`G_16_utility_interrupt/sewage`) == 1 & as.numeric(`G_16_utility_interrupt/cold_water_supply`) == 1) ~ 2,
      (rowSums(across(all_of(crit_9_q1_water_interaptions), .fns = as.numeric), na.rm = T) == 0) ~ 1,
      TRUE ~ NA_real_
    ),
    
    wash_crit_10 = case_when(
      (
        (rowSums(across(all_of(c(crit_7_q1_unimproved)),.fns = as.numeric), na.rm = T) >0 &
           (rowSums(across(all_of(crit_7_q3_level_3),.fns = as.numeric), na.rm = T) == 0)) &
          
          (rowSums(across(all_of(crit_7_q1_improved), .fns = as.numeric), na.rm = TRUE) == 0 & rowSums(across(all_of(c(crit_7_q3_level_2, crit_7_q3_level_3, crit_7_q3_level_4)), .fns = as.numeric), na.rm = TRUE) > 0) |
          
          (rowSums(across(all_of(c(crit_7_q1_improved)),.fns = as.numeric), na.rm = T) == 0 &
             rowSums(across(all_of(c(crit_7_q1_unimproved)),.fns = as.numeric), na.rm = T) == 0 &
             rowSums(across(all_of(c(crit_7_q1_stressed)),.fns = as.numeric), na.rm = T) > 0)) ~ 4,
      
      ((rowSums(across(all_of(c(crit_7_q1_improved,crit_7_q1_stressed)),.fns = as.numeric), na.rm = T) >0 &
          (rowSums(across(all_of(c(crit_7_q1_improved,crit_7_q1_stressed)),.fns = as.numeric), na.rm = T) > 
             rowSums(cbind(as.numeric(`H_8_water_source_dom/bottled_water`),as.numeric(`H_8_water_source_dom/water_kiosk`)), na.rm=T)) &
          (rowSums(across(all_of(c(crit_7_q3_level_3)),.fns = as.numeric), na.rm = T) == 1)) |
         (rowSums(across(all_of(c(crit_7_q1_improved,crit_7_q1_stressed)),.fns = as.numeric), na.rm = T) ==0 &
            rowSums(across(all_of(c(crit_7_q1_unimproved)),.fns = as.numeric), na.rm = T) > 0 &
            (rowSums(across(all_of(c(crit_7_q3_level_3)),.fns = as.numeric), na.rm = T) == 1))
      ) ~ 3,
      
      (rowSums(across(all_of(crit_7_q1_improved), .fns = as.numeric), na.rm = TRUE) > 0 &
         rowSums(across(all_of(crit_7_q3_level_2), .fns = as.numeric), na.rm = TRUE) == 0) ~ 2,
      
      (rowSums(across(all_of(crit_7_q1_improved), .fns = as.numeric), na.rm = TRUE) > 0) ~ 1,
      TRUE ~ NA_real_
    ),
    
    wash_crit_11 =case_when(
      ((rowSums(across(all_of(c(crit_7_q1_unimproved)),.fns = as.numeric), na.rm = T) >0 &
          (rowSums(across(all_of("H_9_enough_water/flushing_toilet"),.fns = as.numeric), na.rm = T) == 0)) &
      ...
      ((rowSums(across(all_of(c(crit_7_q1_improved,crit_7_q1_stressed)),.fns = as.numeric), na.rm = T) >0 &
          (rowSums(across(all_of(c(crit_7_q1_improved,crit_7_q1_stressed)),.fns = as.numeric), na.rm = T) > 
             rowSums(cbind(as.numeric(`H_8_water_source_dom/bottled_water`),as.numeric(`H_8_water_source_dom/water_kiosk`)), na.rm=T)) &
          (rowSums(across(all_of(c('H_9_enough_water/cooking', 'H_9_enough_water/laundry_and_cleaning',
                                   'H_9_enough_water/personal_hygiene')),.fns = as.numeric), na.rm = T) == 1)) |
         (rowSums(across(all_of(c(crit_7_q1_improved,crit_7_q1_stressed)),.fns = as.numeric), na.rm = T) ==0 &
            rowSums(across(all_of(c(crit_7_q1_unimproved)),.fns = as.numeric), na.rm = T) > 0 &
            (rowSums(across(all_of(c('H_9_enough_water/cooking', 'H_9_enough_water/laundry_and_cleaning',
                                     'H_9_enough_water/personal_hygiene')),.fns = as.numeric), na.rm = T) == 1))
      ...
      rowSums(across(all_of(crit_7_q1_undef),.fns = as.numeric), na.rm = T)>0 |
        rowSums(across(all_of(crit_7_q3_undef),.fns = as.numeric), na.rm = T)>0|
        is.na(H_8_water_source_dom)~ NA_real_
    )) 


data.list$main$lsg_wash_v3 <- make_lsg(data.list$main,
                               crit_to_4plus = c('wash_crit_1','wash_crit_2','wash_crit_3'),
                               crit_to_4 = c('wash_crit_4','wash_crit_7'),
                               crit_to_3 = c('wash_crit_5','wash_crit_6'),
)


# data$main$lsg_wash %>% table()
# 
# 
# (data$main$lsg_wash == data$main$wash_crit_1) %>% table()
# (data$main$lsg_wash == data$main$wash_crit_2) %>% table()
# (data$main$lsg_wash == data$main$wash_crit_3) %>% table()
# (data$main$lsg_wash == data$main$wash_crit_4) %>% table()
# (data$main$lsg_wash == data$main$wash_crit_5) %>% table()
# (data$main$lsg_wash == data$main$wash_crit_6) %>% table()
# (data$main$lsg_wash == data$main$wash_crit_7) %>% table()
# (data$main$lsg_wash == data$main$wash_crit_8) %>% table()
# (data$main$lsg_wash == data$main$wash_crit_9) %>% table()
# (data$main$lsg_wash == data$main$wash_crit_10) %>% table()
# (data$main$lsg_wash == data$main$wash_crit_11) %>% table()
# table(data$main$wash_crit_11)

# data$main %>% select(H_2_water_source,H_5_water_assess,H_6_wash_crit_1 water_treat,H_3_1_water_fetch_int,wash_crit_2) %>% View()
# data$main <- data$main %>%
#   dplyr::select(c('wash_crit_1', 'wash_crit_2', 'wash_crit_7'))
# 
# openxlsx::write.xlsx(data$main, 'data.xlsx')
