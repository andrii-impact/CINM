...
protection <- data.list$main %>%
  mutate(
    ...
    safety = case_when(
      (
        (hh_raions_fl == "HHs living in raions close to the frontline or russian border") &
        (f2_conflict_cnt > 0)
      ) ~ 5,
      (
        (hh_raions_fl == "HHs living in raions close to the frontline or russian border") &
      ...
      (
        ...
        (
          f2_lvl4_cnt > 0 &
          hh_raions_fl == "HHs living in raions away from the frontline or russian border"
        )
      ) ~ 3,
      (
        (f2_lvl2_cnt > 0 & (f2_lvl3_cnt + f2_lvl4_cnt) == 0) |
        (f2_conflict_cnt > 0 & hh_raions_fl == "HHs living in raions away from the frontline or russian border")
      ) ~ 2,
      (
        f2_lvl1_cnt > 0
      ) ~ 1,
      f2_undef_cnt > 0 | is.na(`F_2_gen_safety_concerns/none`) | is.na(hh_raions_fl) ~ NA,
      TRUE ~ -1,
    ),
